...
In our case the primary node will act as the active server and it will serve Zimbra services like web server, smtp, imap, etc. The secondary node will be idle just waiting for primary node to fail and bring Zimbra services online when that happens. Not to mention that secondary node also mirrors Zimbra data partition in the background thanks to DRBD.
