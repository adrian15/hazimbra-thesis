...
Drbd configuration settings among others establish a \textit{10 megabits synchronisation} rate using one of the DRBD available protocols (\textit{C}). It also defines that on node named \textit{primary} we define a new DRBD \textit{device} named \textit{drbd0} which it's going to track \textit{/dev/sda3} partition thanks to it's meta-disk partition found in \texit{/dev/sda2}. The same setting is also specified for node named secondary. The only difference is ip \textit{address} where each one of the nodes have DRBD server listening to.
